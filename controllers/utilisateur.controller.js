...
const Login = async (req, res) => {
  try {
    const utilisateur = await UtilisateurModel.findByCredentials(
      req.body.mail,
...
const toutesRessourcesDeUtilisateur = async (req, res) => {
  const user = await UtilisateurModel.find({ _id: req.params.id }).populate(
    "ressources"
  );
  console.log(user);
  if (!user) {
...
const follow = async (req, res) => {
  ...
  if (!utilisateur) {
    ...
    var idexistant = await AbonnementModel.find({
      utilisateur: utilisateur._id,
      abonnement: "62a9da694dc6033c7098a68b",
    });
    if (idexistant == null || idexistant == "") {
      //ON CREE LE FOLLOW
      const follow = new AbonnementModel({
      ...
      var abonnement = await UtilisateurModel.findById({
        _id: "62a9da694dc6033c7098a68b",
      });
